...
// `eden_signs.txt` and `eden_spawn.txt` if they are there, and writes the
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (f == "--name")         { opt.name = next("a name"); name_set = true; }
        else if (f == "--force")          force = true;
        else if (f == "--dry-run")        dry_run = true;
        else if (f == "--zip")            opt.zip = true;
        else if (f == "--base-profile")   base_profile = next("default|none|FILE");
    ...
    if (!name_set) opt.name = basename_of(dir);

    // ── the base profile ────────────────────────────────────────────────────
    std::string err;
    if (base_profile == "default")   opt.profile = eden_default_profile();
