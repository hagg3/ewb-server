...
struct ExportOptions {
    ...
    // Header fields the server does not store. Defaults here; `eden_apply_origin`
    // replays the source world's own values from `eden_origin.txt` (stage 5.6).
    int32_t seed = 0;
    float   yaw  = 0.0f;
    uint8_t skycolors[16] = {0};
    bool    have_home = false;              ///< false: `home` mirrors `spawn`
    Spawn   home{0, 0, 0};
    std::optional<int32_t> version;         ///< the source's header version, a hint
    bool    prefer_256 = false;             ///< the source was 256z: keep it so
...
///
/// `prefer_256` is a tie-breaker, not a floor the caller can lower: a world that
/// was 256z and has since lost everything tall stays 256z, but a 64z origin
/// never caps a world that has grown (that would silently drop cells).
inline int eden_choose_z_ceiling(const WorldStore& store, const std::vector<Sign>& signs,
                                 int force_z, bool prefer_256 = false) {
    if (force_z == 64)  return 63;
    if (force_z == 256) return 255;
    if (prefer_256) return 255;
    bool tall = false;
...
// ── origin replay (stage 5.6) ───────────────────────────────────────────────

/// What on the command line / in the world directory already decided a field.
/// A pinned field is never overwritten by the origin sidecar: the operator's
/// explicit word, and an *edited* live `eden_spawn.txt`, outrank a snapshot taken
/// at import time.
struct OriginPins {
    bool name = false, seed = false, yaw = false, spawn = false, profile = false;
};

struct OriginReplay {
    std::vector<std::string> keys;      ///< the origin keys that took effect, in file order
    bool custom_profile_unpinned = false;   ///< imported with `--base-profile FILE`, none given now
};

/// Fold an origin sidecar into `opt`. Pure; the CLI owns reading the file.
inline OriginReplay eden_apply_origin(const EdenOrigin& o, const OriginPins& pin,
                                      ExportOptions& opt) {
    OriginReplay r;
    if (o.name && !pin.name)  { opt.name = *o.name; r.keys.push_back("name"); }
    if (o.seed && !pin.seed)  { opt.seed = *o.seed; r.keys.push_back("seed"); }
    if (o.pos) {
        // `eden_spawn.txt` holds two decimals, the header a full float. A spawn
        // file that still says what the origin's `pos` says to the file's own
        // precision has not been edited, so the exact value goes back; one that
        // differs is the operator's live choice and wins.
        const bool unedited = pin.spawn && format_spawn_line(*o.pos) == format_spawn_line(opt.spawn);
        if (!pin.spawn || unedited) {
            opt.spawn = *o.pos;
            opt.have_spawn = true;
            r.keys.push_back("pos");
        }
    }
    if (o.home)               { opt.home = *o.home; opt.have_home = true; r.keys.push_back("home"); }
    if (o.yaw && !pin.yaw)    { opt.yaw = *o.yaw; r.keys.push_back("yaw"); }
    if (o.version)            { opt.version = *o.version; r.keys.push_back("version"); }
    if (o.height && *o.height == 256) { opt.prefer_256 = true; r.keys.push_back("height"); }
    if (o.sky) {
        std::copy(o.sky->begin(), o.sky->end(), opt.skycolors);
        r.keys.push_back("sky");
    }
    if (o.base_profile && !pin.profile) {
        if (*o.base_profile == "default")   { opt.profile = eden_default_profile(); r.keys.push_back("base-profile"); }
        else if (*o.base_profile == "none") { opt.profile = eden_empty_profile();   r.keys.push_back("base-profile"); }
        else r.custom_profile_unpinned = true;
    }
    return r;
}

/// The header's `version` field. The parser treats `>= 5` as authoritative 256z
/// (`eden_detect_chunk_size`), so a replayed value is honoured only when it
/// agrees with the z format actually written — otherwise a world edited from
/// 256z down to 64z would be mis-sized by the next reader. Disagreeing (or
/// absent) falls back to what every specimen of the format carries: 4, or 5.
inline int32_t eden_header_version(const std::optional<int32_t>& replay, int z_ceiling) {
    const bool tall = z_ceiling == 255;
    if (replay && ((*replay >= 5) == tall)) return *replay;
    return tall ? 5 : 4;
}
...
inline bool eden_export_world(const WorldStore& store, const std::vector<Sign>& signs,
    ...
    out.z_ceiling  = eden_choose_z_ceiling(store, signs, opt.force_z, opt.prefer_256);
    ...
    if (index.empty()) {
        ...
        if (opt.have_spawn && eden_spawn_in_range(opt.spawn)) {   // a float this far out would not survive int32_t()
    ...
    // field that needs no rename. `home` has no server equivalent: it is the
    // origin sidecar's value when there is one, else it mirrors `pos` so an
    // editor opening the file has a sane set point.
    const Spawn hm = opt.have_home ? opt.home : sp;
    wr_f32(f, sp.x); wr_f32(f, sp.y); wr_f32(f, sp.z);
    wr_f32(f, hm.x); wr_f32(f, hm.y); wr_f32(f, hm.z);
    wr_f32(f, opt.yaw);
    wr_u64(f, dir_offset);
    {
    ...
    // replayed source version is used only where it agrees with that.
    wr_i32(f, eden_header_version(opt.version, out.z_ceiling));
