...
| Lost | Why | Fix |
|---|---|---|
| seed, yaw, `home`, sky palette | the server stores none of them; the header gets defaults (`home` mirrors the spawn) | `--seed` / `--yaw` now; roadmap stage 5.6 replays the source world's own values |
