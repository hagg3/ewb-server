...
//   * **header fidelity** (stage 5.6): an origin sidecar replays seed / yaw / home /
//     sky / version / name so import → export reproduces the source header
//     byte-for-byte apart from `dir_offset`; the pins (`--name`, `--seed`,
//     `--yaw`, a live `eden_spawn.txt`, `--base-profile`) outrank it; a replayed
//     version never contradicts the z format
...
/// A source world whose header carries a value in every field the server cannot
/// store — none of them the export defaults.
static std::vector<uint8_t> fixture_loud_header(int bands, int32_t version) {
    WorldSpec w;
    w.name = "Loud Header";
    w.seed = -424242;
    w.yaw = 2.71828f;
    w.pos[0] = 65601.37f; w.pos[1] = 47.92f; w.pos[2] = 65590.25f;
    w.home[0] = 65530.5f; w.home[1] = 12.0f;  w.home[2] = 65544.75f;
    w.version = version;
    w.chunks.push_back({4096, 4096, build_chunk(bands, [&](int lx, int ly, int z) -> Voxel {
                            if (lx == 5 && ly == 5 && z == 40) return {12, 2};
                            return base_profile(z);
                        }), 0});
    return build_world(w);
}

static ExportOptions replayed(const EdenWorld& src, const std::string& profile = "default",
                              OriginPins pins = OriginPins{}) {
    ExportOptions o;
    eden_apply_origin(eden_origin_from(src, profile), pins, o);
    return o;
}

/// The header, with `dir_offset` (bytes 32..40) blanked and the never-parsed
/// spans (content hash 96..132, tail 148..192) zeroed — what the exit criterion
/// says must survive.
static std::vector<uint8_t> comparable_header(const std::vector<uint8_t>& f) {
    std::vector<uint8_t> h(f.begin(), f.begin() + EDEN_HEADER_BYTES);
    std::fill(h.begin() + 32, h.begin() + 40, uint8_t(0));
    std::fill(h.begin() + 96, h.begin() + 132, uint8_t(0));
    std::fill(h.begin() + 148, h.end(), uint8_t(0));
    return h;
}

static void test_header_fidelity() {
    for (int bands : {4, 16}) {
        const int32_t version = bands == 16 ? 5 : 4;
        const std::vector<uint8_t> src = fixture_loud_header(bands, version);
        const EdenWorld sw = eden_load(src);
        const std::string tag = bands == 16 ? "256z: " : "64z: ";

        ExportResult r;
        const std::vector<uint8_t> out = export_bytes(import_to_store(src), {}, replayed(sw), r);
        CHECK(!out.empty(), (tag + "export succeeds").c_str());
        CHECK(comparable_header(out) == comparable_header(src),
              (tag + "import -> export reproduces the header byte-for-byte").c_str());

        // ...and without the sidecar it does not (the 5.5 behaviour), so the
        // check above is actually testing the replay.
        ExportResult plain;
        ExportOptions o;
        o.name = "Loud Header";
        const std::vector<uint8_t> dflt = export_bytes(import_to_store(src), {}, o, plain);
        CHECK(comparable_header(dflt) != comparable_header(src),
              (tag + "without the sidecar the header falls back to defaults").c_str());
    }
}

static void test_origin_pins() {
    const EdenWorld sw = eden_load(fixture_loud_header(4, 4));
    const EdenOrigin origin = eden_origin_from(sw, "default");

    ExportOptions o;
    o.name = "cli-name"; o.seed = 7; o.yaw = 0.25f;
    o.spawn = {65000.f, 30.f, 65000.f}; o.have_spawn = true;
    OriginPins pins;
    pins.name = pins.seed = pins.yaw = pins.spawn = true;
    const OriginReplay rp = eden_apply_origin(origin, pins, o);
    CHECK(o.name == "cli-name" && o.seed == 7 && o.yaw == 0.25f && o.spawn.x == 65000.f,
          "pins: explicit name / seed / yaw / live spawn outrank the origin");
    CHECK(o.have_home && o.home.x == 65530.5f, "pins: home has no pin; the origin supplies it");
    bool named = false;
    for (const std::string& k : rp.keys) if (k == "name" || k == "seed" || k == "pos") named = true;
    CHECK(!named, "pins: a pinned key is not reported as replayed");

    // A spawn file that only says the origin's pos to two decimals is unedited:
    // the exact float goes back. An edited one still wins.
    ExportOptions u;
    u.spawn = {65601.37f, 47.92f, 65590.25f}; u.have_spawn = true;
    OriginPins us; us.spawn = true;
    eden_apply_origin(origin, us, u);
    CHECK(u.spawn.x == 65601.37f && u.spawn.z == 65590.25f && u.spawn.y == 47.92f,
          "pins: an unedited spawn file yields to the origin's exact pos");
    ExportOptions e;
    e.spawn = {65001.f, 47.92f, 65590.25f}; e.have_spawn = true;
    eden_apply_origin(origin, us, e);
    CHECK(e.spawn.x == 65001.f, "pins: an edited spawn file still wins");

    // No spawn pin: the origin's pos becomes the header position.
    ExportOptions q;
    eden_apply_origin(origin, OriginPins{}, q);
    CHECK(q.have_spawn && q.spawn.x == 65601.37f, "pins: with no live spawn the origin's pos is used");

    // The profile: `none` is replayed unless pinned, `custom` only warns.
    ExportOptions n;
    eden_apply_origin(eden_origin_from(sw, "none"), OriginPins{}, n);
    CHECK(n.profile.layers.empty(), "profile: an origin of `none` is replayed");
    ExportOptions np;
    OriginPins pp; pp.profile = true;
    eden_apply_origin(eden_origin_from(sw, "none"), pp, np);
    CHECK(np.profile.layers.size() == 33, "profile: --base-profile outranks the origin");
    ExportOptions c;
    const OriginReplay cr = eden_apply_origin(eden_origin_from(sw, "custom"), OriginPins{}, c);
    CHECK(cr.custom_profile_unpinned && c.profile.layers.size() == 33,
          "profile: `custom` warns and changes nothing");
    ExportOptions cp;
    CHECK(!eden_apply_origin(eden_origin_from(sw, "custom"), pp, cp).custom_profile_unpinned,
          "profile: `custom` with --base-profile given is not a warning");
}

static void test_origin_version_and_height() {
    // A replayed version never contradicts the z format actually written.
    CHECK(eden_header_version(std::nullopt, 63) == 4 && eden_header_version(std::nullopt, 255) == 5,
          "version: no origin -> 4 / 5");
    CHECK(eden_header_version(3, 63) == 3, "version: an older 64z version is replayed");
    CHECK(eden_header_version(2, 255) == 5, "version: a sub-5 version on a 256z file is not");
    CHECK(eden_header_version(6, 255) == 6, "version: a newer 256z version is replayed");
    CHECK(eden_header_version(5, 63) == 4, "version: >= 5 on a 64z file would be read as 256z");

    // 256z origin, world since emptied of everything tall: stays 256z on auto.
    WorldStore flat;
    flat.set(65536, 40, 65536, 12, 0);
    ExportOptions o;
    o.name = "w";
    o.prefer_256 = true;
    ExportResult r;
    export_bytes(flat, {}, o, r);
    CHECK(r.z_ceiling == 255, "height: a 256z origin keeps a now-low world 256z");
    // ...but an explicit --z 64 still wins, and a 64z origin never caps a tall world.
    o.force_z = 64;
    export_bytes(flat, {}, o, r);
    CHECK(r.z_ceiling == 63, "height: --z 64 outranks the origin");
    WorldStore tall;
    tall.set(65536, 200, 65536, 12, 0);
    ExportOptions o64;
    o64.name = "w";
    eden_apply_origin(eden_origin_from(eden_load(fixture_loud_header(4, 4)), "default"),
                      OriginPins{}, o64);
    export_bytes(tall, {}, o64, r);
    CHECK(r.z_ceiling == 255 && r.cells_dropped_height == 0,
          "height: a 64z origin never caps a world that has grown tall");

    // And the header a replayed sub-5 version leaves still re-imports as 256z.
    ExportOptions o2;
    o2.name = "w";
    o2.version = 2;
    ExportResult r2;
    const EdenWorld back = eden_load(export_bytes(tall, {}, o2, r2));
    CHECK(back.z_ceiling == 255 && back.hdr.version == 5,
          "height: the file still reads back 256z under a stale replayed version");

    // An empty world with an out-of-range origin pos must not misplace its chunk.
    ExportOptions oe;
    oe.name = "w";
    oe.spawn = {1e38f, 33.f, 1e38f};
    oe.have_spawn = true;
    ExportResult re;
    std::string eerr;
    const bool ok = eden_export_world(WorldStore{}, {}, oe, re, eerr);
    CHECK(ok && re.seeded && re.chunks == 1, "empty world: an out-of-range spawn falls back to centre");
}
...
static void test_cli() {
    ...
    // ── 5.6: eden_origin.txt through the real tools ─────────────────────────
    if (!file_exists("./eden_import")) {
        std::fprintf(stderr, "SKIP: ./eden_import not built; origin CLI checks skipped\n");
        return;
    }
    const std::string src = "/tmp/eden_export_test_loud.eden";
    const std::string wd  = "/tmp/eden_export_test_loud_world";
    run("rm -rf " + src + " " + wd + " " + out + " && mkdir -p " + out, o);
    {
        const std::vector<uint8_t> f = fixture_loud_header(4, 4);
        std::ofstream(src, std::ios::binary).write(reinterpret_cast<const char*>(f.data()),
                                                   std::streamsize(f.size()));
    }
    CHECK(run("./eden_import " + src + " --out " + wd + " --yes", o) == 0, "origin cli: import");
    CHECK(file_exists(wd + "/eden_origin.txt"), "origin cli: import writes eden_origin.txt");

    CHECK(run("./eden_export " + wd + " --out " + out + "/a.eden", o) == 0, "origin cli: export");
    CHECK(o.find("origin: " + wd + "/eden_origin.txt (") != std::string::npos &&
              o.find("seed") != std::string::npos,
          "origin cli: the summary names the origin and what it replayed");
    {
        const std::string a = slurp(src), b = slurp(out + "/a.eden");
        const std::vector<uint8_t> av(a.begin(), a.end()), bv(b.begin(), b.end());
        CHECK(comparable_header(av) == comparable_header(bv),
              "origin cli: import -> export reproduces the source header");
    }

    // --no-origin restores the 5.5 behaviour, and --seed / --name outrank the file.
    CHECK(run("./eden_export " + wd + " --out " + out + "/n.eden --no-origin", o) == 0 &&
              o.find("origin: none") != std::string::npos,
          "origin cli: --no-origin ignores the sidecar");
    CHECK(run("./eden_export " + wd + " --out " + out + "/s.eden --seed 99 --name Other", o) == 0,
          "origin cli: flags with an origin present");
    {
        const std::string b = slurp(out + "/s.eden");
        const EdenWorld back = eden_load(std::vector<uint8_t>(b.begin(), b.end()));
        CHECK(back.hdr.seed == 99 && back.hdr.name == "Other" && back.hdr.yaw == 2.71828f,
              "origin cli: --seed / --name pin their fields, the rest still replays");
    }

    // A malformed sidecar, an explicit one that is missing, and contradictory flags.
    { std::ofstream bad(wd + "/eden_origin.txt"); bad << "seed: banana\n"; }
    CHECK(run("./eden_export " + wd + " --out " + out + "/b.eden", o) == 1 &&
              o.find("origin") == std::string::npos && !file_exists(out + "/b.eden"),
          "origin cli: a malformed sidecar is a refusal that writes nothing");
    CHECK(run("./eden_export " + wd + " --out " + out + "/b.eden --no-origin", o) == 0,
          "origin cli: --no-origin steps past a malformed sidecar");
    CHECK(run("./eden_export " + wd + " --out " + out + "/c.eden --origin /tmp/eden_nope.txt", o) == 1,
          "origin cli: an explicit --origin that is missing is a refusal");
    CHECK(run("./eden_export " + wd + " --out " + out + "/c.eden --origin x --no-origin", o) == 2,
          "origin cli: --origin with --no-origin is a usage error");

    run("rm -rf " + src + " " + wd + " " + out, o);
}
...
int main() {
    ...
    test_zip_and_limits();
    test_header_fidelity();
    test_origin_pins();
    test_origin_version_and_height();
    test_cli();
